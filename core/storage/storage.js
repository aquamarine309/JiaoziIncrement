...
export const GameStorage = {
  ...
  backupDataKey(saveSlot, backupSlot) {
    return `ji-backupSave-${saveSlot}-${backupSlot}`;
  },

  backupTimeKey(saveSlot) {
    return `ji-backupTimes-${saveSlot}`;
  },
  ...
  import(saveData) {
    ...
    if (this.checkPlayerObject(newPlayer) !== "") {
      Modal.message.show("存档加载失败(格式无法识别或无效)。");
      return;
    }
  ...
  checkPlayerObject(save) {
    ...
    if (save === undefined || save === null) return "存档解析失败(格式无效)";
    // Right now all we do is check for the existence of an jiaozi prop, but if we wanted to do further save
    // verification then here's where we'd do it
    if (save.jiaozi === undefined) return "存档中没有饺子";
    ...
    return `${quantify("NaN 属性", invalidProps.length)} 已找到:
      ${invalidProps.join(", ")}`;
  },
  ...
  canSave(ignoreSimulation = false) {
    ...
    const isSimulationAnimationActive = player.isSimulationAnimationActive
    return !isSimulating && !isSimulationAnimationActive;
  },
  ...
  exportAsFile() {
    ...
    download(
      `JI Save, Slot ${GameStorage.currentSlot + 1}${saveFileName} #${player.options.exportedFileCount} \
(${this.exportDateString}).txt`, save);
    GameUI.notify.info("成功下载当前存档");
  },
  ...
  exportBackupsAsFile() {
    ...
    download(
      `JI Save Backups, Slot ${GameStorage.currentSlot + 1} #${player.options.exportedFileCount} \
(${this.exportDateString}).txt`, GameSaveSerializer.serialize(backupData));
  ...
  loadPlayerObject(playerObject) {
    ...
    Languages.find(player.options.language).setAsCurrent(true);
