...
class MakerState {
  ...
  tick(diff) {
    ...
    const _diff = (this.tier === 1) ? diff : (diff / 2)
    const production = this.productionForDiff(_diff)
    if (this.tier === 1) Currency.jiaozi.add(production)
    else Maker(this.tier - 1).add(production)
  }
  ...
  get isAffordable() {
    ...
    return Currency.jiaozi.gte(this.cost)
  }
  
  get rate() {
    if (this.tier >= Makers.maxTier) return DC.D0
    return Maker(this.tier + 1).productionPerSecond.div(this.amount.clampMin(1)).div(5)
  }
...
export function buySingleMaker(tier) {
  ...
  Currency.jiaozi.value = Currency.jiaozi.value.minus(maker.cost)
  maker.add(1, true)
  onBuyMaker(tier)
}
...
export function buyMaxMaker(tier, bulk = Infinity) {
  ...
  const maxBought = maker.costScale.getMaxBought(maker.bought, Currency.jiaozi.value, 1)
  if (maxBought === null) return
  let buying = maxBought.quantity
  if (buying > bulk) buying = bulk
  Currency.jiaozi.subtract(Decimal.pow10(maxBought.logPrice))
