...
const pluralDatabase = new Map([
  ["energy", "energy"],
  ["Energy", "Energy"],
  ["Dumpling Energy", "Dumpling Energy"],
  ["day", "days"],
  ["Day", "Days"]
]);
 ...
 * @param {string} [unit]         - this is only used to format Chinese
...
window.quantify = function quantify(name, value, places, placesUnder1000, formatType = format, unit) {
  if (name === undefined || value === undefined) throw "[quantify warn] Arguments must be defined";

  const number = formatType(value, places, placesUnder1000);
  const plural = pluralize(name, value, undefined, unit);
...
window.quantifyInt = function quantifyInt(name, value) {
  if (name === undefined || value === undefined) throw "[quantifyInt warn] Arguments must be defined";
